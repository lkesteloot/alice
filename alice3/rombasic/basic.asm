...
; stolen from intmini.asm...
                .ORG $0000
;------------------------------------------------------------------------------
; Reset

RST00           DI                       ;Disable interrupts
                JP       MONINIT            ;Initialize Hardware and go
...
; continue from Grant's basic.asm
...
TSTBRK: 
        ...
        ; searle: RST     10H             ; Get the key into A
        call    conin           ; alice
        CP      ESC             ; Escape key?
        JR      Z,BRK           ; Yes, break
        CP      CTRLC           ; <Ctrl-C>
...
GETINP:
        ; searle: RST	    10H             ;input a character
...
; alice3: conout:	;console character output from register c

	; ld	a,c		;get the char
	out	(propeller_port),a		;out to port
	ret

        ; searle: JP      $0008           ; output a char
