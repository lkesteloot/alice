...
;
	...
	OR	A
	CALL	NZ,DSKSEL	;dont do needless selects.
;
...
HL2DE:	LD	A,(HL)
	...
	RET	
;
;   Compute (HL)=(TBUFF)+(A)+(C) and get the byte thats here.
;
EXTRACT: LD	HL,TBUFF
	ADD	A,C
	CALL	ADDHL
...
DIRECT3: JP	Z,DIRECT9	;terminate if no more names.
	LD	A,(RTNCODE)	;get files position in segment (0-3).
	RRCA	
	RRCA	
	RRCA	
	...
	JP	C,DIRECT8	;we dont list them.
	POP	DE
...
DIRECT6: LD	A,B
	...
	JP	NZ,DRECT65
	POP	AF		;yes, dont print spaces at the end of a line.
	PUSH	AF
	CP	3
	JP	NZ,DRECT63
	...
	AND	7FH
	CP	' '
	JP	Z,DIRECT7	;dont print spaces.
...
UNKWN9:	CALL	RESETDR		;inproper format.
	JP	SYNERR
UNKWN0:	LD	BC,BADLOAD	;read error or wont fit.
	CALL	PLINE
	JP	GETBACK
...
GETECHO: CALL	GETCHAR		;input a character.
	...
	RET	C		;no, a regular control char so dont echo.
...
OUTCHAR: LD	A,(OUTFLAG)	;check output flag.
	OR	A		;anything and we wont generate output.
	JP	NZ,OUTCHR1
	PUSH	BC
	CALL	CKCONSOL	;check console (we dont care whats there).
	POP	BC
	PUSH	BC
...
OUTCHR1: LD	A,C		;update cursors position.
	LD	HL,CURPOS
	CP	DEL		;rubouts dont do anything here.
	RET	Z
...
TRKSEC:	LD	HL,(FILEPOS)	;get position of last accessed file
	...
	LD	(CKSUMTBL),HL	;whats it doing here too?
;
...
STNREC1: LD	C,A
	...
	LD	(HL),A		;and set fcbs next record byte.
	EX	DE,HL
...
MOREFLS: LD	HL,(FILEPOS)	;we are here.
	EX	DE,HL
	LD	HL,(SCRATCH1)	;and dont go past here.
	LD	A,E		;compute difference but dont keep.
	SUB	(HL)
	INC	HL
	LD	A,D
...
DIRDMA1: LD	C,(HL)
	...
	JP	SETDMA
;
;   Move the directory buffer into users dma space.
;
...
; 1 then it will be set (dont use anyother values).
;
STBITMAP: PUSH	DE
...
BITMAP2: LD	C,0FFH		;read next file name in directory
	...
	LD	(STATUS),A
BITMAP3: LD	C,1		;now set this files space as used in bit map.
	CALL	SETFILE
...
FNDNXT2: LD	A,C		;are we done with the compare?
	...
	CP	'?'		;dont care about this character?
	JP	Z,FNDNXT4
	LD	A,B		;get bytes position in fcb.
	CP	13		;dont care about the thirteenth byte either.
	JP	Z,FNDNXT4
	CP	12		;extent byte?
	LD	A,(DE)
	JP	Z,FNDNXT3
...
MOVEWORD: LD	A,(HL)		;check for a zero word.
	...
	LD	A,(DE)
	LD	(HL),A
	DEC	DE		;dont disturb these registers.
	DEC	HL
	RET	
;
...
CLOSEIT5: INC	DE		;bump to next item in table.
	...
	ADD	HL,BC
	LD	A,(DE)
	CP	(HL)		;directorys extent already greater than the
...
RDSEQ:	LD	A,1		;set sequential access mode.
	LD	(MODE),A
RDSEQ1:	LD	A,0FFH		;dont allow reading unwritten space.
	LD	(RDWRTFLG),A
...
USERDMA: DEFW	080H		;users dma address (defaults to 80h).
;
